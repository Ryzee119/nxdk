...
bool xinputh_set_config(uint8_t dev_addr, uint8_t itf_num)
{
    ...
    xid_itf->epin_buf = BUS_ALLOC(xid_itf->epin_size);
    xid_itf->epout_buf = BUS_ALLOC(xid_itf->epout_size);
...
void xinputh_close(uint8_t dev_addr)
{
    ...
    for (uint8_t inst = 0; inst < xinput_dev->inst_count; inst++)
    {
        ...
        xinputh_interface_t *xid_itf = get_instance(dev_addr, inst);
        BUS_FREE(xid_itf->epin_buf);
        BUS_FREE(xid_itf->epout_buf);
    }
